...
export default function EditUserModal({ isOpen, onClose, customer, onUserUpdated }) {
    ...
    useEffect(() => {
        if (customer) {
            setFormData({
                ...
                zipCode: customer.zipCode || customer.shippingAddress?.zipCode || "",
                city: customer.city || customer.shippingAddress?.city || "",
                country: customer.country || customer.shippingAddress?.country || "",
            });
        }
    }, [customer]);

    // Auto-fetch City/State if PIN exists but they are empty (for backward compatibility or sync issues)
    useEffect(() => {
        if (formData.zipCode && formData.zipCode.length === 6 && (!formData.city || !formData.country)) {
            const autoFetch = async () => {
                setPincodeLoading(true);
                const result = await lookupPincode(formData.zipCode);
                if (result.success) {
                    setFormData(prev => ({
                        ...prev,
                        city: prev.city || result.district || result.city,
                        country: prev.country || result.state,
                    }));
                }
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`/api/admin/customer/update/${customer._id}`, {
                method: "PUT",
                headers: {
                    ...
                    <form onSubmit={handleSubmit} className="px-6 py-6">
                        ...
                        <div className="border-t border-gray-100 my-4 pt-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            disabled={loading}
                                            className={`block w-full rounded-md border ${errors.zipCode ? "border-red-300 ring-1 ring-red-300" : "border-gray-300"} px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed`}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                        disabled={loading}
                                        className="block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
                                        placeholder="City"
                                    />
                                </div>
                                <div>
                            ...
                            <div className="mb-6">
                                ...
                                <textarea
                                    ...
                                    disabled={loading}
                                    rows={2}
                                    className="block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed resize-none"
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                onClick={handleClose}
                                disabled={loading}
                                className="flex-1 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                disabled={loading}
                                className="flex-1 rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center"
                            >
                                {loading ? (
                                    <>
